    ...
    private void loadFoodData(File file) {
        try (FileReader reader = new FileReader(file)) {
            ...
            if (!rootElement.isJsonArray()) {
                Log.e("FOOD_LOAD", "유효하지 않은 JSON 형식 (배열 아님): " + rootElement.toString());
                Toast.makeText(this, "분석된 음식 정보가 유효하지 않습니다.", Toast.LENGTH_SHORT).show();
                titleText.setText("음식 인식 실패");
                saveButton.setText("다시 시도");
                return;
            }

            JsonArray foodArray = rootElement.getAsJsonArray();
            ...
            for (JsonElement elem : foodArray) {
                ...
                if (!foodObj.has("foodName") || !foodObj.has("nutrition")) continue;

                String name = foodObj.get("foodName").getAsString();
                JsonObject nutrition = foodObj.getAsJsonObject("nutrition");

                if (!nutrition.has("calories")) continue;
                double kcal = nutrition.get("calories").getAsDouble();
            ...
            Log.e("FOOD_LOAD", "food_data 읽기 실패: " + e.getMessage(), e);
            Toast.makeText(this, "음식 데이터 파일 로딩 실패", Toast.LENGTH_SHORT).show();
            titleText.setText("데이터 파일 오류");
            saveButton.setText("다시 시도");
        }
    }
